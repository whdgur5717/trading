...
- Deploy override: 기본 배포 대상은 `turbo query affected --packages back front` 결과로
  정하고, merged PR label `deploy:front`, `deploy:back`, `deploy:all`이 있으면 해당
  platform 배포를 강제합니다. 배포 차단 label은 사용하지 않습니다.
...
- GitHub Actions affected deploy shaping:
  ...
  - workflow job permission에는 merged PR label 조회를 위해 `pull-requests: read`만
    추가했습니다.
  - GitHub repository labels `deploy:front`, `deploy:back`, `deploy:all`을 만들고 REST
    API로 존재를 확인했습니다.
  - PR label override는 `deploy:front`가 front만, `deploy:back`이 back만,
    `deploy:all`이 둘 다 deploy output을 true로 만드는지 로컬 Node simulation으로
    확인했습니다.
  ...
  - 임시 worktree에서 workflow 순서와 같이 `pnpm build` 뒤 deploy target JSON을
    ...
    둘 다 대상으로 잡혔습니다.
  - GitHub Actions의 no-change `front:build` cache miss는 affected 판단 문제가 아니라
    `.open-next/**` Turbo output restore 실패였습니다. artifact에
    `.open-next/.../front/node_modules/next` symlink와 그 하위 파일이 같이 들어가 tar
    restore가 `Cannot extract through symlink`로 실패했습니다.
  - `front#build.outputs`는 `.open-next/**`를 유지하되
    `!.open-next/**/node_modules/**`를 제외하도록 정정했습니다. 독립 producer/consumer
    clone에서 `.turbo` cache를 옮겨도 `front:build cache hit`, `Cached: 2 cached, 2 total`
    로 복원됐고, 복원된 `.open-next`로 Wrangler dry-run이 통과했습니다.
...
- GitHub Actions run에서 composite action 기반 affected deploy와 OpenNext cache restore가
  실제로 통과하는지 확인합니다.
