...
0. **Announce yourself once, before the first poll.** The relay keeps a _sticky_
...
3. **Answer in ONE shot — speed matters.** Each click should feel instant, so
   ...
   **3b. Whole-transition recipe swap (`refineType === "replace"` only — no file
   reads).** Match the inferred usage to ONE recipe below (this list _is_ the
   ...
   **empty** `suggestions` array with a short `summary`.
   ...
   calls for; if the transition already _is_ the right recipe, return empty.
...
DOM scan only sees each element's _current_ computed transition — it can't tell
...
**Be fast.** The `raw.timings` are already accurate for each element's _current_
...
(CSSOM) that drive that element across _all_ states (base + open + close), with
...
2. **Split each component into phases** — usually `open` and `close` (a hover-only
   ...
   provided timings; the _opposite_ phase often lives on a different selector
...
1. **Locate the real declaration in the source.** The `selector` is a DOM-path
   _hint_, not necessarily the source selector. Use the `component` hint and search
...
| `reference`   | _(replace only, optional)_ the transitions.dev reference file the user should paste for the full recipe, e.g. `"transitions-dev/06-modal.md"`.                                                                                                                                   |
| `reason`      | one sentence of _why_, in usage terms                                                                                                                                                                                                                                            |
