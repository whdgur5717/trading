...
export const autocomplete = tv({
  slots: {
    ...
    inputGroup:
      "group grid min-h-14 w-full grid-cols-[minmax(0,1fr)_auto] items-end gap-x-3 gap-y-1 rounded-xl bg-surface-muted px-4 py-3 text-left font-sans text-ink transition-colors duration-150 ease-standard outline-none focus-within:bg-surface focus-within:outline-2 focus-within:outline-offset-2 focus-within:outline-accent hover:bg-surface data-disabled:cursor-not-allowed data-disabled:bg-disabled data-disabled:text-disabled-foreground data-invalid:outline-2 data-invalid:outline-offset-2 data-invalid:outline-tease data-popup-open:bg-surface",
    input:
    ...
    content:
      "z-50 w-(--anchor-width) max-w-(--available-width) rounded-2xl bg-surface-popup p-3 text-ink shadow-surface inset-shadow-surface outline-none data-open:animate-popover-in",
    results: "grid gap-1",
    viewport: "grid gap-1",
