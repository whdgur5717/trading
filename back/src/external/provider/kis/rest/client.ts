...
import { quotationMarketCode } from "./schema"
import { KisRestRequest } from "./request"
import { candlesMapper, marketDayMapper, priceMapper } from "./schema"

@Injectable()
export class KisRestClient {
  constructor(private readonly requestProvider: KisRestRequest) {}

  price(query: PriceQuery): Promise<Result<Price, ExternalError>> {
  ...
  async candles(query: CandlesQuery): Promise<Result<Candle[], ExternalError>> {
