...
import { OPENDART_AUTH_FAILURE_STATUS, OPENDART_NO_DATA_STATUS } from "./schema"
import {
  companyMapper,
  corpCodeMapSchema,
...
export class OpendartClient {
  ...
  corpCode(symbol: string): Result<string, ExternalError> {
    const parsedSymbol = stockSymbolSchema.safeParse(symbol)

    if (!parsedSymbol.success) {
      return err(
        externalErrors.dataNotFound({
          provider: "opendart",
    ...
    const corpCode = corpCodeMap.get(parsedSymbol.data)
  ...
  async company(
    corpCode: string
  ): Promise<Result<CompanyProfile, ExternalError>> {
  ...
  ): Promise<Result<MarketDisclosure[], ExternalError>> {
  ...
  ): Promise<Result<FinancialAccount[], ExternalError>> {
  ...
  private parseBody<TSchema extends z.ZodType>(
    ...
    endpoint: string,
    schema: TSchema
  ): Result<z.output<TSchema> | null, ExternalError> {
  ...
  ): Promise<Result<HttpResponse, ExternalError>> {
