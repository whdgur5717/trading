...
- 주식 데이터는
  ...
  활용한다.
- KIS 연동 작업 전에는 공식 Open API spec을 확인한다.
...
- 루트 `pnpm prepare:api`는 `pnpm generate:openapi` 후 `pnpm generate:api`를
  실행한다.
- `nest-cli.json`은 `@nestjs/swagger` compiler plugin을 사용한다.
- `back/scripts/generate-openapi.mjs`가 빌드된 `dist` 앱을 읽어
  `packages/api-client/openapi.json`을 생성한다.
- `packages/api-client/openapi.json`은 직접 손으로 고치지 않는다.
- API 계약이 바뀌면 schema/DTO/controller를 먼저 고치고 OpenAPI JSON을 다시
  생성한다.
- 프론트 생성 클라이언트는 `packages/api-client/openapi.json`을 입력으로 사용한다.
- 계약 변경 시 프론트 생성 코드도 함께 갱신한다.
- 성공/실패 계약이 바뀌면 OpenAPI와 프론트 생성 클라이언트를 함께 갱신한다.
...
- KIS API를 다룰 때는 공식 spec을 기준으로 판단한다.
...
- 테스트 이름은 개발자 이외의 팀원(PM, PO 등)이 봐도 이해할 수 있는 행위 중심의
  한국어 문장으로 작성한다.
